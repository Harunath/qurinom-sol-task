...
import { useUserSignup } from "@/lib/store/signup";
import { signIn } from "next-auth/react";

const DEV_SIMULATE = process.env.NEXT_PUBLIC_SIMULATE_OTP === "true";

export async function purgeUserSignupPersist() {
	// delete persisted entry: { name: "user-signup", version: 2, ... }
	await useUserSignup.persist.clearStorage();
	// reset in-memory state too
	useUserSignup.getState().clearAll();
}

export default function MerchantRegisterPage() {
	const router = useRouter();
	const { phone, otpToken, devOtp, setStart, setVerified } = useUserSignup();
		...
		<form onSubmit={onSubmit} className="space-y-4">
			...
			<p className="text-center text-xs text-gray-500">
				...
				<button
					...
					onClick={purgeUserSignupPersist}>
					Reset
				</button>
			</p>
			{DEV_SIMULATE && devOtp ? (
