	...
	providers: [
		...
		CredentialsProvider({
			...
			async authorize(credentials) {
				...
				if (!user || !user.hashedPassword || user.role !== Role.MERCHANT) {
					throw new Error("Invalid phone or password");
				}
				...
				return {
					...
					name: user.name ?? null,
					storeCompleted: user.stores.length > 0,
				};
			},
		}),
	],
	...
	callbacks: {
		...
		async jwt({ token, user, trigger, session }) {
			if (user && user.phone) {
				...
				if (merchant) {
					...
					token.storeCompleted = Boolean(
						merchant.stores.length > 0 ? true : false
					);
				}
				...
				if (trigger === "update" && session) {
					...
					if ("storeCompleted" in session)
						token.storeCompleted = Boolean((session as any).storeCompleted);
					if ("role" in session)
		...
		async session({ session, token }) {
			if (session.user && token) {
				...
				session.user.role = token.role as "MERCHANT";
				session.user.phoneVerified = token.phoneVerified as boolean;
				session.user.name = (token.name as string | null) ?? null;
				session.user.storeCompleted = Boolean(token.storeCompleted);
			}
			return session;
		},
